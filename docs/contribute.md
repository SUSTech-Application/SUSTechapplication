...
- 撰写经验分享帖子（推荐使用 Markdown 格式）
- 在仓库中[创建新 Issue](https://github.com/SUSTech-Application/SUSTechapplication/issues/new)，并将你的帖子作为附件上传。
- 如果你不希望使用 GitHub，也可以直接将帖子发送给维护者。

::: warning 为什么不推荐直接上传文件？

- 飞跃手册维护者们需要处理大量帖子以及对网站进行升级，维护。尤其是将帖子从其他格式转换为 Markdown 十分耗费时间。通过 GitHub 提交 PR 进行分享，将大大减轻维护者们的负担。
